package com.insigma.cloud.code.serviceimpl.common.svercord;

import com.insigma.cloud.code.dao.common.svercord.ApiSvercordMapper;
import com.insigma.cloud.code.service.common.svercord.ApiSvercordService;
...
public class ApiSvercordServiceImpl implements ApiSvercordService {
	...
	public String saveSVerCode(SVerCode sVerCode) {
		//将历史验证码设置为无效
		apiSvercordMapper.deleteSVerCode(sVerCode);
		//有效状态 1-有效
		...
		//保存验证码信息
		apiSvercordMapper.saveSVerCode(sVerCode);
	...
	public boolean checkMobileVerCode(String mobile, String vercode, String optype) {
		...
		List<SVerCode> list = apiSvercordMapper.getMobileVerCode(sVerCode);
		if (list.size() == 0) {
			return false;
		}
		...
		if(!result.getVercode().equals(vercode)){
			...
			//更新验证码状态等信息
			apiSvercordMapper.updateSVerCode(result);
			return false;
		}
