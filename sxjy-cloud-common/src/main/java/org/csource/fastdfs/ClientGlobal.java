...
public class ClientGlobal
{
	...
	*/
		public static void init(InputStream fis ) throws FileNotFoundException, IOException, MyException
		{
			...
			pro.load(fis);
	  		String[] szTrackerServers;
				...
				g_connect_timeout = pro.getProperty("connect_timeout")==null?DEFAULT_CONNECT_TIMEOUT:Integer.parseInt(pro.getProperty("connect_timeout"));
	  		...
	  		g_network_timeout = pro.getProperty("network_timeout")==null?DEFAULT_NETWORK_TIMEOUT:Integer.parseInt(pro.getProperty("network_timeout"));
	  		...
	  		g_charset = pro.getProperty("charset");
	  		if (g_charset == null || g_charset.length() == 0)
	  		{
	  			g_charset = "ISO8859-1";
	  		}
	  		
	  		szTrackerServers = pro.getProperty("tracker_server").split(",");
	  		...
	  		g_tracker_http_port = pro.getProperty("http.tracker_http_port")==null?80:Integer.parseInt(pro.getProperty("http.tracker_http_port"));
	  		g_anti_steal_token =   pro.getProperty("http.anti_steal_token")==null?false:Boolean.parseBoolean(pro.getProperty("http.anti_steal_token"));
	  		if (g_anti_steal_token)
	  		{
	  			g_secret_key =pro.getProperty("http.secret_key");
	  		}
		}
