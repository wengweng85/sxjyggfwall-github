...
var rc = {
   ...
   setTab:function(name,cursel,n){
    ...
    open_imgage_upload_page:function(file_bus_id,file_bus_type,upload_callback){
     ...
     //打开excel上传页面
    open_excel_file_upload_page:function(excel_batch_excel_type,upload_callback){
 		if(excel_batch_excel_type&&upload_callback){
    		layer.open({
	   		  ...
	   		  shade: 0.8,
	   		  area: ['30%', '40%'],
	   		  content: contextPath+"/common/fileload/toExcelFileUpload?excel_batch_excel_type="+excel_batch_excel_type+"&upload_callback="+upload_callback //iframe的url
 		    });
    	}else{
    		layer.alert('上传文件需要的业务编号、业务类型、回调函数不能为空,请确认');
    	}
...
var closableTab = {
	...
	addTab:function(tabItem){ //tabItem = {id,name,url,closable}
		...
		if(!$('#'+id)[0]){
			var li_tab = '<li role="presentation" class="" id="'+id+'"><a href="#'+container+'"  role="tab" data-toggle="tab" style="position: relative;padding:2px 20px 2px 15px;font-color:blue;">'+tabItem.name+"&nbsp;";
			if(tabItem.closable){
