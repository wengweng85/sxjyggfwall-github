...
function openPersonRegist() {
  layer.open({
    ...
    area: ['810px', '350px'], //宽高
    content: contextPath + '/pregist/to',
  });
}
// 个人用户手机号注册弹窗
function openPersonMobileRegist() {
    layer.open({
        type: 2,
        closeBtn: true, // 关闭按钮
        shadeClose: false, // 是否点击遮罩关闭
        ...
        scrollbar: false, // 是否允许浏览器出现滚动条
        area: ['810px', '350px'], //宽高
        content: contextPath + '/pregist/toMobile',
    });
}

// 企业用户注册弹窗
function openCompanyRegist() {
  layer.open({
    type: 2,
    closeBtn: true, // 关闭按钮
    shadeClose: false, // 是否点击遮罩关闭
    ...
    scrollbar: false, // 是否允许浏览器出现滚动条
    area: ['810px', '350px'], //宽高
    content: contextPath + '/cregist/to',
  });
}

// 个人用户登录弹窗
function openPersonLogin() {
  layer.open({
    type: 2,
    title: '个人用户登录',
    shadeClose: true,
    closeBtn: 1, //不显示关闭按钮
    area: ['480px', '320px'],
    content: contextPath + '/common/person/login'
  });
}
...
function openCompanyLogin() {
  layer.open({
    ...
    closeBtn: 1, //不显示关闭按钮
    area: ['480px', '320px'],
    content: contextPath + '/common/enterprise/login'
  });
}
